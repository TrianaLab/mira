...
.PHONY: tui-cast
tui-cast: ## Re-record the docs/assets/tui/*.gif casts from a live Mira
	@# Needs `make demo` running in another terminal, plus Pillow and a
	@# monospaced TTF. Nothing in CI runs this: the GIFs are committed, and they
	@# are regenerated by hand when what they show changes. Look at the result —
	@# a layout that has lost content still renders perfectly well.

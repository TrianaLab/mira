...
exactly one: `required`. Every other job earns its authority by being in that
job's transitive `needs:` closure. That design has one failure mode, and it is
silent: add a job, forget to wire it into the gate, and it now runs, goes red,
and merges anyway. Nobody notices for months.

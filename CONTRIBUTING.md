...
mechanism — read [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §0 and §1.
§0 lists the mechanisms that did not survive contact with the formats, and
...
**Before you open a PR: `make check`.** That is the whole checklist. If it is
green locally it is green in CI, and if it is not, that is a bug in the
Makefile worth reporting on its own.
...
count and a stripped binary size, and `docs/ARCHITECTURE.md` §11 scores binary
...
and the §11 table in the same diff. `make deps` is the other half: `cargo deny`
...
number is a hope. Numbers in the README's "What is true today" must have been
measured on the machine that ran them, so if your change moves one, re-measure
...
Open an issue before a large change. Not for process: because §0 may already
...
licence — Apache-2.0 §5, inbound equals outbound. There is no CLA and no

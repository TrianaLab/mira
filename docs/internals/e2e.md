...
**For:** contributors running Mira against real data. Where a *new* test belongs
is [Testing architecture](testing.md); to look at Mira working rather than
measure it, [See it work](../demo.md).

Four jobs, and none needs the others:

| To | Sections |
| --- | --- |
| drive a live binary by hand | 1, 2 |
| reproduce a published number | 3, 4 |
| exercise the operator on a real cluster | 5 |
| point somebody else's telemetry at it | 6 |
...
Without `--demo` it is the load harness of section 3, keyed by connection:
cumulative totals from eight writers in *one* series would fall backwards.
...
**`localhost` can answer from the wrong Mira**: with the section 6 stack up,
...
--addr HOST:PORT
--demo           the realistic generator instead of the harness (section 1)
--selftest       assert the demo's invariants in memory and exit (section 1)
```
...
Findings, not instructions: why each one lands where it does is
[architecture section 11](../architecture/performance.md), and this is the run
behind it.
...
## 5. A tier the operator built, on a real cluster

...
There is no independent versioning to compute, which is why there is no version
bot, no plan file and no transaction ledger here — see [Why not
changesets](#why-not-changesets) at the bottom, because that is the question
this page gets asked most.
...
3. **Open a normal PR with both.** The point is to make CI run `drift`, `chart`
   and `docs` against the bumped tree. The release PR was always the valuable
   half of a version bot; the bot that types it was not.
...
the next patch — which is the accepted trade. A disposable-registry rehearsal is
a lot of shell to de-risk one event at a version nobody has installed.

## Why not changesets

The sibling project [pacto](https://pacto.run) drives its releases with
[changesets](https://github.com/changesets/changesets): contributors write
`.changeset/*.md` fragments, a bot opens a version PR, merging it publishes.
That is the right machinery *there* — pacto publishes ten units across two
independently-versioned lines, so the hard part is computing which versions move
and keeping a ten-way publish transactional.

Mira publishes three coordinates from one linear DAG with one version number.
Changesets' core value is worth exactly zero here: the arithmetic it would
perform is `0.0.1` → `0.0.2`. What it would cost is concrete:

- **A root `package.json` and `node_modules`.** The README and
  [Install](../install.md) both promise no Node toolchain is needed to build
  Mira, and the UI bundle is committed precisely so that stays true. (The
  dependency-budget gate is *not* the objection — `check_drift.py` runs
  `cargo tree` and cannot see npm.)
- **A third-party action holding a write token.** `changesets/action` needs
  `contents: write` and `pull-requests: write` against a top-level
  `permissions: contents: read`. This tree has twice refused an action for less.
- **A gate that only exists in YAML.** Every gate here is a `make` target a
  contributor can run; a `changeset version` step in a workflow structurally
  cannot be.
- **A job with nowhere to live.** It publishes nothing and runs on push-to-main,
  so it fits neither `release.yml`'s terminal-job closure nor `ci.yml`'s required
  closure. It would need a third publishing workflow.

The one genuine argument for it is that a changeset body *cannot* degrade to
autogenerated notes — and mira does not use Conventional Commits, so
`--generate-notes` is a poor fallback. A hand-written `## [X.Y.Z]` section is
file-derived in exactly the same way and is already written; step 2 of [Cutting
a release](#cutting-a-release) is the whole of that guarantee.

Two mechanisms from pacto are worth knowing about and are deliberately *not*
here. A **durable OCI ledger** with adopt/conflict states would make a crash
between push and record resumable; `release.yml` says no in a comment that names
the ceiling — two runs racing on the same tag — and the upgrade path, which is
registry-side immutable tags rather than a ledger. A **disposable-registry
rehearsal** that really pushes and signs is covered above. Both are re-proposed
often enough to be worth writing down as decided.

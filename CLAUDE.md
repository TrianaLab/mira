...
[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) §0 and §1 before changing anything
structural — §0 lists the mechanisms from the original brief that do not survive
contact with the formats, and re-proposing one of them is the most common way to
waste a session.

## The five principles

They are constraints, not aspirations, and each has a mechanism (§1):
...
The README states the crate count and binary size, and §11 scores binary size as
an axis. Before and after adding any dependency:

```sh
cargo tree -p mira --edges normal --prefix none | awk '{print $1" "$2}' | sort -u | wc -l
...
- **README "What is true today"** is a promise. Every bullet must be true of the
  committed code, and every number must have been measured on this machine. Move
  anything else to "What is not true yet".

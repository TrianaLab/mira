...
//!   the one failure mode an index is not allowed to have (section 7.4);
...
//!
//! No proptest. The dependency budget is a stated product property (section 11) and
...
impl Rng {
    ...
    /// By value, not by reference: every table here holds `&'static str` or a
    /// tuple of them, so a reference would be a reference to a reference and
    /// every call site would carry a `*` that the MSRV needs and clippy on a
    /// current toolchain reports as redundant.
...
fn store(rng: &mut Rng, root: &std::path::Path, base: i64) -> Vec<Row> {
    ...
    for seq in 1..=1 + rng.below(4) {
        ...
        block::publish(root, "logs", node, seq, 0, &sealed).unwrap();
    }
    rows
}

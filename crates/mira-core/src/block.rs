...
mod tests {
    ...
    fn free_reports_bytes_beside_the_fraction() {
        ...
        // Two `statfs` calls, so not the same instant: a volume with anything
        // else running on it moves between them, and asserting exact equality
        // here fails on a busy laptop rather than on a real divergence.
        let drift = (f - free_fraction(&d).unwrap()).abs();
        assert!(drift < 0.01, "the same fraction, give or take {drift}");

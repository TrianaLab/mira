...
///
/// Every other enumerable string in the schema is `u16` and seals the block on
/// overflow (see [`DICT_CAP`]). Attribute *values* are the one string here that
/// is genuinely unbounded — a trace id, a URL, a GenAI prompt — so a `u16` would
/// turn a high-cardinality tenant into a block sealed every few thousand rows.
/// `u32` costs about one percent of the compressed size over `u16` and removes
/// the failure mode: 4 billion distinct values do not fit in a 32 MiB block.
...
pub static ATTRS: LazyLock<SchemaRef> = LazyLock::new(|| {
    Arc::new(Schema::new(vec![
        ...
        // Dictionary-encoded, not plain `Utf8`, and it is the only value column
        // that is: measured on one real 393,216-row block it takes the
        // cold-block ratio from 12.9x to 66.2x on its own, because attribute
        // values are where the repetition in telemetry lives — the same
        // `http.route`, the same pod name, the same status text, a few hundred
        // distinct strings across a few hundred thousand rows. `u32` rather
        // than `key`'s `u16` because values are unbounded in principle and the
        ...
        // docs/architecture/performance-cost.md, "What compresses and what does not".
...
/// boundaries, which OTLP wall-clock reads do not. See docs/architecture/data-layout.md.

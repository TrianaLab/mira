...
//!
//! The Helm chart's version is the same class of problem and so it lives here
//! too: `charts/mira/Chart.yaml` restates the workspace version three times over
...
/// helm-docs from `chart.version`, so `make helm-docs-check` already catches
/// those — what nothing else catches is Chart.yaml disagreeing with the crate it
/// installs.
const CHART_YAML: &str = "charts/mira/Chart.yaml";
...
fn version_sites() -> Vec<(&'static str, Regex)> {
    vec![
        ...
        // Three lines in one file, which is three chances to bump two of them.
        // The image tag is the expensive one: Artifact Hub reads
        // `artifacthub.io/images` to attach a security report, so a stale tag
        // shows the previous release's CVEs against this release's listing.
        (CHART_YAML, re(r"(?m)^version: (\d+\.\d+\.\d+)$")),
        (CHART_YAML, re(r#"(?m)^appVersion: "(\d+\.\d+\.\d+)"$"#)),
        (
            CHART_YAML,
            re(r"(?m)^\s*image: ghcr\.io/trianalab/mira:(\d+\.\d+\.\d+)$"),
        ),
        (
        ...
        // is a 404 rather than a typo. `--version v?` covers both spellings: the
        // installer takes a tag (`v0.0.2`), helm takes a chart version
        // (`0.0.2`).
        ...
        (
            "docs/install.md",
            re(r"ghcr\.io/trianalab/charts/mira:(\d+\.\d+\.\d+)"),
        ),
    ...
    found.unwrap_or_else(|| {
        eprintln!(
            "error: Cargo.toml has no `version` under `[workspace.package]`. That is \
             where the whole workspace — and the Helm chart — takes its version from."
        );
        std::process::exit(1);
    })
}
...
fn check_version_sites(crate_version: &str, f: &mut Failures) {
    println!("chart:       {crate_version} in Cargo.toml");
            ...
            if g.as_str() != crate_version {
                f.fail(format!(
                    "{rel}:{} says {}, but the workspace is at {crate_version}.\n    \
                     One binary, one chart, one number — a second is only ever a \
    ...
    match read(ARTIFACTHUB_REPO_YML) {
        Err(_) => f.fail(format!(
            "{ARTIFACTHUB_REPO_YML} is gone. release.yml pushes it to \
             `ghcr.io/trianalab/charts/mira:artifacthub.io`, and without it the chart \
...
/// generated, and `make bump` regenerates them straight after. Nor does it touch

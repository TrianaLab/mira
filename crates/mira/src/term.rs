...
//! crates, 5.62 MiB) is a stated property of the product.
...
/// sites in [`enter`](Term::enter) stop being four lines each.
...
pub(crate) mod tests {
    ...
    fn one_key_arrives_in_as_many_spellings_as_there_are_terminals() {
        // `None` is [`Parsed::Need`]: what is there could still grow into a
        // longer sequence, so no key comes out of it yet.
        ...
        // A CSI with no final byte yet is the "could still grow" answer, not a
        // key: the table below would otherwise be reading half a sequence.
        ...
        for (bytes, want) in [
            ...
            (b"\n", Some(Key::Enter)),
            // Nothing at all is not a key either: `key` is called in a loop that
            // polls again, and a `Parsed::Key` here would be one nobody pressed.
            (b"", None),
            // Half of a CSI and half of an SS3, which is how they arrive when
            // the read lands mid-sequence.
            (b"\x1b[", None),
            (b"\x1bO", None),
        ] {
    ...
    /// The other half of the same refusal: if a `termios` call fails anyway,
    /// `enter` reports why instead of pressing on with a terminal it has half
    /// configured. Provoked on a pipe, since `enter` turns away everything that
    /// is not a tty before it reaches either call.
    #[test]
    fn a_failed_termios_call_carries_the_os_error() {
        let mut fds = [-1; 2];
        // SAFETY: `pipe` writes exactly the two `c_int`s this array holds.
        assert_eq!(unsafe { libc::pipe(fds.as_mut_ptr()) }, 0);
        // SAFETY: all-zero is a valid `termios`, as in `enter` — and here both
        // calls fail without writing it at all.
        let mut t: libc::termios = unsafe { std::mem::zeroed() };
        // SAFETY: `fds[0]` is open until the `close` below, and both calls get
        // a live, correctly typed `termios`.
        let get = ok_or_errno(unsafe { libc::tcgetattr(fds[0], &mut t) });
        // SAFETY: as above; `tcsetattr` only reads it.
        let set = ok_or_errno(unsafe { libc::tcsetattr(fds[0], libc::TCSANOW, &t) });
        // SAFETY: both fds were opened by the `pipe` above and are owned by
        // nothing else, so this is the only close of either.
        unsafe {
            libc::close(fds[0]);
            libc::close(fds[1]);
        }
        for r in [get, set] {
            // ENOTTY, not a generic "it did not work": the message the user
            // sees is the difference between a redirect and a broken terminal.
    ...
    /// EINTR is a resize and means "nothing to read"; every other `poll` error
    /// has to come back as one, or the draw loop spins on a syscall that is
    /// failing every time and never says so.
    ///
    /// `poll_in` polls fd 0 with nfds 1, which nothing outside the process can
    /// make fail — except that both Linux and macOS reject an nfds above
    /// RLIMIT_NOFILE with EINVAL, and the soft limit can be dropped to zero.
    /// That is process-wide and a process that can open nothing is no use to
    /// the rest of the suite, so it happens in a re-exec of this one test.
    #[test]
    fn a_poll_that_fails_for_anything_but_a_signal_is_an_error() {
        ...
        // SAFETY: `rlimit` is two integers, and `getrlimit` writes exactly the
        // one live struct it is handed.
        ...
        // SAFETY: `setrlimit` only reads the struct, for the call's duration.
        assert_eq!(unsafe { libc::setrlimit(libc::RLIMIT_NOFILE, &none) }, 0);
        let got = poll_in(0);
        // Put it back before anything — the assertion's own panic machinery
        // included — needs a descriptor. Lowering the soft limit is reversible
        // precisely because the hard limit above was left alone.
        // SAFETY: as above.
    ...
    fn the_terminal_half_runs_against_a_real_pty() {
        ...
        // Destructured rather than held as a `Pty`: the assertions below want
        // the names in their format strings, and a format argument on its own
        // line is a region that only a failing run ever executes.
        let Pty {
            screen,
            err,
            stalled,
    ...
    /// What [`drive`] saw: everything the child wrote to the pty, everything it
    /// wrote to stderr, the marker it stopped answering at if it stopped at one,
    /// and whether the trailing marker ever arrived.
    ...
    /// Run one test of this binary on the far side of a real pty.
    ///
    /// Open a pty, re-exec this same test binary against the slave side, and
    ...
    /// [`crate::tui`] drives its own event loop through here for the same reason
    /// this module does: a loop that calls `Term::enter` needs somewhere to enter.
    pub(crate) fn drive(
        test: &str,
        ...
        // `*const winsize`, which is what `&ws` is, and `fd` is borrowed from
        // `slave`, still open here — it is dropped after the spawn.
        ...
        let stalled = script
            ...
            .map(|(marker, _)| *marker);
        // Otherwise it sits in `poll_in(-1)` for the rest of the afternoon. One
        // expression rather than an `if`, so the healthy run — the only one CI
        // ever takes — still executes the cleanup line.
    ...
    fn on_the_pty() {
        ...
        // A tty nobody sized — a bare `posix_openpt`, `script -q /dev/null` on
        // some runners — answers TIOCGWINSZ with zeros rather than failing, and
        // a frame laid out for zero columns is a blank screen. 80x24 instead.
        ...
        for (ws, want) in sizes.iter().zip([(80, 24), (120, 40)]) {
            // SAFETY: TIOCSWINSZ's variadic argument is `*const winsize`, which
            // is what `ws` is, and fd 1 is the pty slave — the same fd `size`
            // reads back through. Setting it back afterwards is what leaves the
            // rest of this function at the size the parent asked for.
        ...
        // Two shapes a real terminal will not produce on demand, injected last
        // because both of them break the pty this test is standing on.
        //
        // A window with no size: `size` reads it back off a *successful* ioctl,
        // and zero columns is what a detached or not-yet-sized tty answers.
        // Every column computation in `tui.rs` divides by that number.
        ...
        // SAFETY: as the parent's TIOCSWINSZ above — `*const winsize` is the
        // argument this request takes, and fd 1 is this process's pty slave.
        ...
        // EOF on stdin, which `read` reports as zero bytes on a `poll` that says
        // there is something there. Reported as ^C because that is the only
        // answer that ends the draw loop — anything else spins on a descriptor
        // that is ready forever.
        let devnull = std::fs::File::open("/dev/null").unwrap();
        let null_fd = std::os::fd::AsRawFd::as_raw_fd(&devnull);
        // SAFETY: both arguments are open descriptors this process owns. `dup2`
        // closes fd 0 and duplicates `null_fd` onto it; nothing reads fd 0
        // after this but the `key` below.
